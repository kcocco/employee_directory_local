...
function getEmployees(tx) {
	//alert("getemployee");
	//var sql = "SELECT DISTINCT ClinStateCode, count(ClinStateCode) stateCount " + 
	var sql = "SELECT * " + 
				"FROM IVF";
...
function getEmployees_success(tx, results) {
    ...
    for (var i=0; i<len; i++) {
    	var IVF = results.rows.item(i);
		$('#employeeList').append('<li><a href="employeedetails.html?id=' + IVF.ClinStateCode + '">' +
				'<p class="line1">' + IVF.ClinStateCode + '</p>' +
				'<span class="bubble">' + IVF.stateCount + '</span></a></li>');
    }
	setTimeout(function(){
		scroll.refresh();
	},100);
